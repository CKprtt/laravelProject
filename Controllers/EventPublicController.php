...
class EventPublicController extends Controller
{
    ...
    {
        $hallEvents = Event::where('type_hall', 'Yes')->orderBy('start_date', 'desc')->get();
        $nonHallEvents = Event::where('type_hall', 'No')->orderBy('start_date', 'desc')->get();

        return view('events.indexEvent', compact('hallEvents', 'nonHallEvents'));
    }
    ...
    public function showEvent($id)
    {
        ...
        $eventRequest = EventRequest::where('events_id', $event->events_id)->first();
        if ($eventRequest !== null) {
            $artistId = $eventRequest->artist_id;
            $artistName = ArtistProfile::where('artist_id', $artistId)->value('artist_name');
        } else {
            $artistName = null;
        }

        $type = $event->type_hall ?? 'No';
        $isHall = ($type === 'Yes');

        $zones = $zoneA = $zoneB = $zoneC = null;
        if ($isHall) {
            $zones = HallZone::orderBy('zones_id')->get();
            foreach ($zones as $zone) {
                $used = TicketBooking::where('events_id', $id)
                    ->where('zones_id', $zone->zones_id)
                    ->sum('qty');
                $zone->used = (int) $used;

                if ($zone->zones_name === 'A') $zoneA = $zone;
                elseif ($zone->zones_name === 'B') $zoneB = $zone;
                elseif ($zone->zones_name === 'C') $zoneC = $zone;
            }
        }

        $myBooking = null;
        if (Auth::check()) {
            $myBooking = TicketBooking::where('users_id', Auth::user()->id)
    ...
    // //     $hallEvents    = Event::where('type_hall', 'Yes')->orderBy('start_date', 'asc')->get();
    // //     $nonHallEvents = Event::where('type_hall', 'No')->orderBy('start_date', 'asc')->get();
