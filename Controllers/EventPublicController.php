...
/**
 * Controller นี้ใช้สำหรับ "ฝั่งผู้ใช้ทั่วไป (Public/User)"
 * - แสดงหน้า "รายการอีเวนต์ทั้งหมด" (/events)
 * - แสดงหน้า "รายละเอียดอีเวนต์" + คำนวณคงเหลือโซน (/events/{id})
 *  - ตาราง events ของคุณใช้คอลัมน์ events_name, start_at, end_at (เราทำ accessor ใน Model แล้วให้เรียก $event->name ได้)
 *  - ตาราง hall_zones มี zones_name, zones_capacity
 *  - ตาราง ticket_bookings เก็บการจอง (ไม่ล็อกที่นั่งรายที่ แต่จำกัดจำนวนต่อโซน)
 */
class EventPublicController extends Controller
{
    ...
    public function index()
    {
        ...
        */
        return view('events.index', ['hallEvents'=> $hallEvents,'nonHallEvents' => $nonHallEvents,]);
    }
    ...
    public function show($id)
    {
        ...
        if ($isHall) { // ทำงานเฉพาะเมื่อเป็น Hall
            ...
            foreach ($zones as $z) {
                $used = TicketBooking::where('event_id', $id)
                    ->where('zone_id', $z->id)
                    ->sum('qty'); 
                $z->used = (int) $used; /*00/40 เช็คยอดจอง -> เพิ่มขึ้น 01/40*/
                
                if ($z->zones_name === 'A') {
                    $zoneA = $z;
                } elseif ($z->zones_name === 'B') {
                    $zoneB = $z;
                } elseif ($z->zones_name === 'C') {
                    $zoneC = $z;
                }
            }
        }
        ...
        return view('events.show', compact('event','zones','myBooking','artistName', 'zoneA', 'zoneB', 'zoneC','isHall'));
    }
}